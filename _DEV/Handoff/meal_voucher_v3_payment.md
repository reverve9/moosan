...
- **구현 완료** — commit `309120a feat(coupon): meal voucher payment processing and settlement` (dev push 완료)
- **검증 보류** — 테스트 환경 미가용으로 §7 케이스 A~E 미실행. 추후 결제 가능한 환경에서 시나리오 따라 실행 후 결과 기록 필요.
- 검증 시점에 발급된 테스트 식권 (16장 × 4번호) 그대로 살아있음 — 재발급 불필요.
...
**케이스 E: 다부스 비례 분배 (구현 추가분)**
- [ ] 부스A 4,000 + 부스B 6,000 = 10,000 주문에 8,000원 식권 적용
- [ ] 부스A.voucher_consumed ≈ 3200 (proportional, floor)
- [ ] 부스B.voucher_consumed = 4800 (= 8000 - 3200, 마지막 부스 잔여 흡수)
- [ ] burned = 0, userPaid = 2000
- [ ] SUM(voucher_consumed) === 8000 (정확히 액면가)

**케이스 F: 다부스 + 잔액 소멸 (구현 추가분)**
- [ ] 부스A 2,000 + 부스B 4,000 = 6,000 주문에 8,000원 식권 적용
- [ ] 부스A.voucher_consumed ≈ 2000, voucher_burned=0
- [ ] 부스B.voucher_consumed ≈ 4000, voucher_burned=2000 (마지막 부스에 burned 몰아넣음)
- [ ] PG 우회 (userPaid=0), payment.payment_key=NULL, status='paid'
